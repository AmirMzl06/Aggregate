...
def train_decoder(X_train_feats, y_train, X_test_feats, y_test, tag):
    ...
    yval_t = torch.tensor(y_val, dtype=torch.long, device=device)
    Xte_t = torch.tensor(Xte, dtype=torch.float32, device=device)
    decoder = TwoLayerMLP(
        input_dim=Xtr.shape[1],
        hidden_dim=DECODER_HIDDEN_DIM,
    ...
    for epoch in range(DECODER_EPOCHS):
        ...
        for start in range(0, len(Xtr_t), DECODER_BATCH_SIZE):
            ...
            optimizer.step()
        # -----------------------------------------------------
        # Evaluate ONLY on validation set during training
        # -----------------------------------------------------
        decoder.eval()
        with torch.no_grad():
            val_pred = decoder(Xval_t).argmax(dim=1).cpu().numpy()
        ...
        if (epoch + 1) % 50 == 0 or epoch == 0:
            print(
                f"[{tag}] epoch {epoch + 1}/{DECODER_EPOCHS} | "
    ...
    print(
        f"[{tag}] Best validation accuracy = "
        f"{best_val_acc:.4f} at epoch {best_epoch}"
    )
    # Save BEST decoder, not the last decoder.
    torch.save(
        decoder.state_dict(),
        os.path.join(
            OUT_DIR,
            f"decoder_{tag}_side_classifier.pt"
        )
    )
    # ---------------------------------------------------------
    ...
    for true, p in zip(y_test, pred):
        cm[true, p] += 1
    print(
        f"[{tag}] FINAL TEST accuracy={test_accuracy:.4f} | "
        f"chance={chance:.4f} | "
    ...
    pd.DataFrame(
        cm,
        index=["true_left", "true_right"],
        columns=["pred_left", "pred_right"]
    ).to_csv(
        os.path.join(
            OUT_DIR,
            f"decoder_{tag}_confusion.csv"
        )
    )
    ...
    ax.set_title(
        f"{tag} | test accuracy={test_accuracy:.3f}"
    )
    ...
    return test_accuracy, best_val_acc, best_epoch
...
cebra_acc, cebra_val_acc, cebra_best_epoch = train_decoder(
    X_train_cebra,
    y_train,
    X_test_cebra,
    y_test,
...
acorn_acc, acorn_val_acc, acorn_best_epoch = train_decoder(
    X_train_acorn,
    y_train,
    X_test_acorn,
    y_test,
...
comparison = pd.DataFrame({
    ...
    "best_val_accuracy": [cebra_val_acc, acorn_val_acc],
    "best_epoch": [cebra_best_epoch, acorn_best_epoch]
})
