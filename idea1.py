...
# This controls OUTER adversarial-update negative-embedding gradients.
# The attacked input stays REFERENCE in BOTH arms, as requested.
IDEA1_GAMMA = 0.5  # Change to 0.0 for full stop-gradient; baseline is always 1.0.
...
def experiment_configs(n_neurons):
    if not 0.0 <= IDEA1_GAMMA < 1.0:
        raise ValueError("Use 0 <= IDEA1_GAMMA < 1; baseline already uses 1.")
    common = common_encoder_config()
    return [
        ("BASELINE_gamma1", dict(common, adv_negative_grad_scale=1.0)),
        (f"IDEA1_gamma{IDEA1_GAMMA:g}",
         dict(common, adv_negative_grad_scale=IDEA1_GAMMA)),
    ]
