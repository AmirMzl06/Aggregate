...
def train_branch(
    dataset_name,
    branch_name,  # Updated parameter for dynamic file naming
    train_input_np,
    train_time_np,
    train_target_np,
    test_input_np,
    ...
    # Use branch_name to avoid overwriting files between full and subset trainings
    save_path = os.path.join(out_dir, f"SMD_{dataset_name}_{branch_name}.pth")
    if SAVE_MODELS:
    ...
    print(f"[{branch_name}] Top-K = {k} out of {n_features}")
    print(f"[{branch_name}] Top-K (Jc):    {topk_jf.tolist()}")
    print(f"[{branch_name}] Top-K (Jc-inv): {topk_jfinv.tolist()}")

    raw_attr_path = os.path.join(out_dir, f"SMD_{dataset_name}_{branch_name}_raw_jacobians.pt")
    torch.save(
        {
            "jf": jf_tensor,
            "jf_inv": jfinv_tensor,
    ...
    npz_attr_path = os.path.join(out_dir, f"SMD_{dataset_name}_{branch_name}_attrs.npz")
    if SAVE_NPZ:
        np.savez_compressed(
            npz_attr_path,
    ...
    jf_plot_path, jfinv_plot_path, pair_plot_path = save_model_plots(
        ...
        model_tag=branch_name,  # Using branch_name as tag
        jf_map=jf_map,
        jfinv_map=jfinv_map,
    ...
    decoder_path = os.path.join(out_dir, f"SMD_{dataset_name}_{branch_name}_decoder.pth")
...
def main():
    ...
    for filename in train_files:
        ...
        # 1. Full models: raw data only
        for adv in [False, True]:
            model_name = "ACORN" if adv else "CEBRA"
            branch_name = f"full_{model_name}"
            ...
            # Re-save plot with the final R2 in the title
            save_model_plots(
        ...
        # 2. 8-State Ablation Study (Top-K radical Training)
        print("\n" + "=" * 100)
        print("Starting 8-State Subset Training (Top-K features)")
        print("=" * 100)

        for source_model in ["CEBRA", "ACORN"]:
            for attr_key in ["jf", "jfinv"]:
                topk_idx = full_results[source_model][f"topk_{attr_key}"]
                
                # Subset the input features based on Top-K
                train_x_sub = train_x[:, topk_idx]
                test_x_sub = test_x[:, topk_idx]

                for retrain_adv in [False, True]:
                    retrain_model = "ACORN" if retrain_adv else "CEBRA"
                    branch_name = f"src_{source_model}_attr_{attr_key}_retrain_{retrain_model}"
                    ...
                    res = train_branch(
                        dataset_name=dataset_name,
                        branch_name=branch_name,
                        train_input_np=train_x_sub.astype(np.float32),
                        train_time_np=train_t.astype(np.float32),
                        # Target remains the FULL data to measure complete signal reconstruction
                        train_target_np=train_x.astype(np.float32),
                        test_input_np=test_x_sub.astype(np.float32),
                        test_target_np=test_x.astype(np.float32),
                        adv=retrain_adv,
                    )
        ...
        # 3. Print Summary for Dataset
        print("\n" + "#" * 90)
        print(f" SUMMARY FOR {dataset_name} ".center(90, "#"))
        print("#" * 90)

        for r in global_rows:
            if r["dataset"] == dataset_name:
                if r["setting"] == "full":
                    print(f"FULL   | Model: {r['retrained_model']:>5} | Base R2: {r['base_r2']:.4f}")
                else:
                    print(f"SUBSET | Src: {r['source_model']:>5} | Attr: {r['attr_type']:>6} | "
                          f"Retrained: {r['retrained_model']:>5} | R2: {r['base_r2']:.4f}")
