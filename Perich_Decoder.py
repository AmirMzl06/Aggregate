...
def load_perich_session():
    ...
    print("*** LABELS ARE USED FOR THE DOWNSTREAM DECODER ONLY ***")
...
# =====================================================================
# Decoder: TwoLayerMLP, trained on frozen CEBRA embeddings to predict
# the behavior labels. CEBRA itself never sees Y (see load_perich_session
# note above) -- this is a purely post-hoc readout of the embedding.
# =====================================================================
class TwoLayerMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, output_dim=2, dropout_rate=0.4):
        super().__init__()
...
def train_decoder(Z_train, Y_train, tag):
    ...
    for epoch in range(DECODER_EPOCHS):
        ...
        epoch_loss = 0.0
        for start in range(0, n, DECODER_BATCH_SIZE):
        ...
        epoch_loss /= n
        if (epoch + 1) % DECODER_PRINT_EVERY == 0 or epoch == 0:
            print(f"  epoch {epoch + 1}/{DECODER_EPOCHS} | train MSE: {epoch_loss:.6f}")

    cleanup(Zt, Yt, optimizer)
    return decoder
...
def main():
    ...
    print("FORWARD + INVERSE JACOBIAN + DECODER")
    print("=" * 90)
    print("Session:", f"{DATASET_NAME}{DAY}")
    print("NPZ:", NPZ_PATH)
    ...
    clean_decoder = train_decoder(Z_train_clean, Y_train, tag="CLEAN")
    clean_train_r2 = evaluate_decoder(clean_decoder, Z_train_clean, Y_train, "CLEAN DECODER — TRAIN")
    clean_test_r2 = evaluate_decoder(clean_decoder, Z_test_clean, Y_test, "CLEAN DECODER — TEST")
    torch.save(clean_decoder.state_dict(), os.path.join(OUT, "CLEAN_decoder.pt"))

    del clean_model, clean_decoder
    cleanup(Z_train_clean, Z_test_clean)
    ...
    acorn_decoder = train_decoder(Z_train_acorn, Y_train, tag="ACORN")
    acorn_train_r2 = evaluate_decoder(acorn_decoder, Z_train_acorn, Y_train, "ACORN DECODER — TRAIN")
    acorn_test_r2 = evaluate_decoder(acorn_decoder, Z_test_acorn, Y_test, "ACORN DECODER — TEST")
    torch.save(acorn_decoder.state_dict(), os.path.join(OUT, "ACORN_decoder.pt"))

    del acorn_model, acorn_decoder
    cleanup(Z_train_acorn, Z_test_acorn)
    ...
    print("=" * 90)
    print(f"CLEAN | train mean R2={clean_train_r2:.4f} | test mean R2={clean_test_r2:.4f}")
    print(f"ACORN | train mean R2={acorn_train_r2:.4f} | test mean R2={acorn_test_r2:.4f}")
    decoder_summary = pd.DataFrame({
        "model": ["CLEAN", "ACORN"],
        "train_mean_r2": [clean_train_r2, acorn_train_r2],
        "test_mean_r2": [clean_test_r2, acorn_test_r2],
    })
    decoder_summary_path = os.path.join(OUT, "decoder_r2_summary.csv")
    decoder_summary.to_csv(decoder_summary_path, index=False)
    print("Saved:", decoder_summary_path)
    ...
    print(os.path.join(OUT, "CLEAN_decoder.pt"))
    print(os.path.join(OUT, "ACORN_decoder.pt"))
    print(decoder_summary_path)
