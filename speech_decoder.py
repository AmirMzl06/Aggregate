...
def build_phoneme_vocab(samples):
    vocab = set()
    for _, text, _ in samples:
        vocab.update(text_to_phonemes(text))
    return sorted(vocab)


class PhonemeCharset:
    def __init__(self, phones: List[str]):
        self.idx2sym = ["<BLANK>"] + list(phones)
        self.sym2idx = {p: i + 1 for i, p in enumerate(phones)}
        self.sym2idx["<BLANK>"] = 0

    @property
    def num_classes(self) -> int:
        return len(self.idx2sym)

    def seq_to_int(self, phones: List[str]) -> List[int]:
        return [self.sym2idx[p] for p in phones if p in self.sym2idx]

    def int_to_seq(self, ids: List[int]) -> List[str]:
        return [self.idx2sym[i] for i in ids if i != 0]
...
class BrainToTextPhonemeDataset(Dataset):
    def __init__(self, samples, phoneme_charset):
        self.items = []
        for X, text, day_idx in samples:
            ids = phoneme_charset.seq_to_int(text_to_phonemes(text))
            if len(ids) == 0:
                continue
            x = torch.tensor(X, dtype=torch.float32)
            y = torch.tensor(ids, dtype=torch.long)
...
# =====================================================================
def evaluate_metrics(model, loader, ctc_criterion, device):
    model.eval()
    allLoss = []
    ...
    with torch.no_grad():
        for Xv, yv, Xv_len, yv_len, _ in loader:
            ...
            for i in range(pred_v.shape[0]):
                ...
                total_len += len(true_ids)

    avgLoss = float(np.sum(allLoss) / max(len(loader), 1))
    per = total_edit / max(total_len, 1)
    return avgLoss, per    
