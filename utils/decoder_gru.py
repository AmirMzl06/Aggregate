...
class GRU_Dec(nn.Module):
    ...
    def __init__(self, feat_dim: int, hidden: int, num_outputs: int, nLayers: int = 2, bidir=True,
        ...
        self.out = nn.Linear((2 if bidir else 1) * hidden, num_outputs) 
    ...
    def fit(self, train_x: torch.Tensor, train_y: torch.Tensor, seed: int = 42, adv_steps: int = 10, adv_eps : float= 0.01, adv: bool = False):
        ...
        for epoch in p_bar:
            ...
            with torch.no_grad():
                with self._autocast():
                    pred = self(val_x)
                    if torch.all(torch.isfinite(pred)): 
                        r2 = self._r2_mean(pred, val_y)
                    else:
                        r2 = -100000
                        inf_epochs += 1
                        if inf_epochs > 5:
            ...
            p_bar.set_postfix_str(f" r2:{r2:.3f}")
