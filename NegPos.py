"""
C-CO12 BEHAVIOR-CEBRA experiment: NORMAL CEBRA vs NEGPOS
...
4) Train the same TwoLayerMLP decoder (2500 epochs) and report R^2.
5) Compute and plot encoder Jacobians on TRAIN data only, using
   CEBRA's own attribution API:
       cebra.attribution.init(
...
No ACORN is included.

The train set is sampled as ATTR_CHUNKS chunks of length ATTR_LEN.
For each chunk, CEBRA computes its Jacobian feature map ("jf"), and the
absolute maps are averaged across the train chunks, exactly following the
...
def main():
    ...
    # NEW: Jacobians are computed only from C-CO12 TRAIN neural windows.
    run_train_jacobians(
        normal_model,
        negpos_model,
        X_train,
