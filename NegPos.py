...
1) Train NORMAL CEBRA with behavior labels.
2) Train CEBRA-NegPos with the same behavior labels and random cross-session negatives.
3) Run the same two embedding tests and save PCA 2D/3D plots.
4) Train the same TwoLayerMLP decoder (2500 epochs) for NORMAL and NEGPOS and report R^2.
5) Compute and plot encoder Jacobians on TRAIN data only for NORMAL and NEGPOS,
   using CEBRA's own attribution API.
6) Switch to the standard ACORN-capable CEBRA checkout, train ACORN with
   behavior labels using epsilon=0.5, then train the SAME TwoLayerMLP decoder
   and report test R^2.
7) Print the R^2 values for all three models side by side at the end.

ACORN does NOT get the embedding comparison plots or Jacobian plots in this script;
only its model, decoder, and R^2 result are added.
"""
...
if not CEBRA_DIR.exists():
    raise FileNotFoundError(
        f"CEBRA-NegPos fork not found: {CEBRA_DIR}"
    )
...
# NegPos
EXTRA_NEGATIVE_FRACTION = 0.99999
...
# Jacobian / CEBRA attribution
# Uses CEBRA's own attribution implementation:
#     cebra.attribution.init(name="jacobian-based-batched", ...)
# exactly in the same style as the supplied reference code.
ATTR_CHUNKS = 16
ATTR_LEN = 128
ATTR_BATCH = 16
...
# =====================================================================
# TRAIN-SET JACOBIAN -- CEBRA ATTRIBUTION
# =====================================================================

def compute_cebra_train_jacobian(
    model,
    X_train,
    display_name,
):
    """
    Compute the Jacobian feature map using CEBRA's own attribution API.

    This intentionally follows the supplied reference implementation:
    ...
    Only X_train is used.

    Returns
    -------
    jf_mean : np.ndarray, shape (LATENT_DIM, n_neurons)
        Train-set average absolute forward Jacobian feature map.
    starts : np.ndarray
        Start indices of the train chunks used for attribution.
    """
    print("\n" + "-" * 90)
    ...
    if len(X_train) <= ATTR_LEN:
        raise ValueError(
            f"X_train has only {len(X_train)} samples, "
            f"but ATTR_LEN={ATTR_LEN}."
        )

    # Use the fitted shared encoder exactly like the reference code.
    net = model.solver_.model

    # Defensive handling in case a single-element ModuleList is ever returned.
    if isinstance(net, nn.ModuleList):
        if len(net) != 1:
            raise RuntimeError(
                "Expected a single-session CEBRA encoder for attribution."
            )
        net = net[0]
    ...
    for chunk_idx, start in enumerate(starts, start=1):
        ...
        with torch.enable_grad():
            result = method.compute_attribution_map(
                batch_size=ATTR_BATCH
            )

        if "jf" not in result:
            raise RuntimeError(
                "CEBRA attribution result does not contain key 'jf'. "
                f"Available keys: {list(result.keys())}"
            )

        jf = np.abs(
            np.asarray(result["jf"])
        )

        jf = np.squeeze(jf)

        # Same reduction logic as the supplied CEBRA-attribution reference.
        if jf.shape != (
            LATENT_DIM,
            n_neurons,
        ):
            if jf.ndim < 2:
                raise RuntimeError(
                    f"Unexpected CEBRA Jacobian shape: {jf.shape}"
                )
        ...
        if jf.shape != (
            ...
            raise RuntimeError(
                "After reduction, expected CEBRA attribution Jacobian "
                f"shape {(LATENT_DIM, n_neurons)}, got {jf.shape}."
            )

        jf_sum += (
            jf.astype(np.float64)
            * len(chunk)
        )
        total += len(chunk)

        print(
            f"{display_name}: attribution chunk "
            f"{chunk_idx}/{len(starts)} | "
            f"start={start} | "
            f"jf shape={jf.shape}"
        )
    ...
    print(
        f"{display_name}: final Jacobian shape = "
        f"{jf_mean.shape}"
    )
...
def plot_single_jacobian(
    ...
    plt.colorbar(
        im,
        label="Absolute forward Jacobian",
    )
...
def plot_jacobian_comparison(
    ...
    """
    Separate heatmaps + side-by-side comparison.

    All plots use the SAME color scale so the magnitude comparison between
    NORMAL CEBRA and NEGPOS is visually meaningful.
    """
    vmax = float(
        max(
            np.max(normal_J),
    ...
    normal_path = (
        JACOBIAN_DIR
        / "normal_cebra_train_cebra_jacobian.png"
    )

    negpos_path = (
        JACOBIAN_DIR
        / "negpos_train_cebra_jacobian.png"
    )

    plot_single_jacobian(
        normal_J,
    ...
    fig.suptitle(
        "Train Forward Jacobian -- CEBRA Attribution"
    )
    ...
    cbar.set_label(
        "Absolute forward Jacobian"
    )

    out_path = (
        JACOBIAN_DIR
        / "normal_vs_negpos_train_cebra_jacobian.png"
    )
...
def run_train_jacobians(
    ...
    normal_J, normal_starts = compute_cebra_train_jacobian(
        normal_model,
        X_train,
        "NORMAL CEBRA",
    )

    negpos_J, negpos_starts = compute_cebra_train_jacobian(
        negpos_model,
        X_train,
        "NEGPOS",
    )
    ...
    if not np.array_equal(
        ...
        raise RuntimeError(
            "Normal CEBRA and NegPos attribution used different train chunks."
        )

    if SAVE_JACOBIAN_ARRAYS:
        np.save(
            JACOBIAN_DIR
            / "normal_cebra_train_cebra_jf.npy",
            normal_J,
        )

        np.save(
            JACOBIAN_DIR
            / "negpos_train_cebra_jf.npy",
            negpos_J,
        )

        np.save(
            JACOBIAN_DIR
            / "cebra_attribution_train_chunk_starts.npy",
            normal_starts,
        )
...
# =====================================================================
# ACORN + LABEL + SAME DECODER
# =====================================================================

def _clear_all_cebra_modules():
    """Remove the currently loaded CEBRA fork before switching checkouts."""
...
def import_acorn_cebra_class():
    """
    Switch from CEBRA-NegPos to the standard ACORN-capable CEBRA checkout
    specified by utils.constants.CEBRA_DIR.
    """
    ...
    if not acorn_dir.exists():
        raise FileNotFoundError(
            f"ACORN CEBRA_DIR does not exist: {acorn_dir}"
        )

    _clear_all_cebra_modules()

    # Remove the NegPos checkout and any previous ACORN path, then put
    # the ACORN checkout at highest import priority.
    while str(CEBRA_DIR) in sys.path:
        sys.path.remove(str(CEBRA_DIR))
    ...
    if missing:
        raise RuntimeError(
            "The CEBRA checkout from utils.constants.CEBRA_DIR "
            "does not expose the expected ACORN API. "
...
def build_acorn(
    ...
    """Behavior-conditioned ACORN with the same encoder hyperparameters."""
    return ACORN_CEBRA(
        batch_size=BATCH_SIZE,
        temperature=TEMPERATURE,
        model_architecture=MODEL_ARCH,
...
def train_acorn_and_decoder(
    ...
    """
    Train ACORN with behavior labels, then use exactly the same TwoLayerMLP
    decoder and report test R^2.

    No ACORN PCA or Jacobian is computed here.
    """
    print("\n" + "#" * 100)
    ...
    # BEHAVIOR ACORN: labels are explicitly passed.
    acorn_model.fit(
        X_train,
        Y_train,
    )
    ...
    acorn_train_emb = np.asarray(
        acorn_model.transform(
            X_train.astype(np.float32)
        ),
        dtype=np.float32,
    )

    acorn_test_emb = np.asarray(
        acorn_model.transform(
...
def print_all_three_r2(
    ...
    """
    Final side-by-side test R^2 table.
    """
    names = (
        "NORMAL CEBRA",
        "NEGPOS",
...
def main():
    ...
    # Same foreign-neuron construction as before.
    ...
    # Train extra negatives are matched neuron-by-neuron to C-CO12 TRAIN.
    ...
    # Both models are behavior CEBRA: fit(X_train, Y_train).
    ...
    # Same embedding tests as before.
    test_fake(
        normal_model,
        negpos_model,
        X_test,
    ...
    # Same TwoLayerMLP decoder, 2500 epochs, R^2.
    (
        normal_r2,
        normal_mean,
        negpos_r2,
    ...
    # Jacobians are computed only from C-CO12 TRAIN neural windows.
    run_train_jacobians(
        normal_model,
        negpos_model,
        X_train,
    ...
    # Normal/NegPos are no longer needed after their decoder/Jacobian stages.
    del normal_model
    del negpos_model
    cleanup()

    # Final model: standard behavior-conditioned ACORN.
    acorn_r2, acorn_mean = train_acorn_and_decoder(
        X_train,
        X_test,
        Y_train,
        Y_test,
    ...
    # Keep this as the final result block printed before script exit.
    print_all_three_r2(
        normal_r2,
        normal_mean,
        negpos_r2,
