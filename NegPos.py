...
# # =============================================================================
# # FORK IMPORT (single fork -- no ACORN this time, so no fork-switching needed)
# # =============================================================================
...
# print("\nUsing CEBRA-NegPos(Detach) fork:")
# print(cebra.__file__)

# _params = inspect.signature(CEBRA.__init__).parameters
# _required = {"extra_negatives", "extra_negative_fraction"}
# _missing = _required.difference(_params)
# if _missing:
#     raise RuntimeError(f"Wrong CEBRA fork loaded. Missing NegPos arguments: {sorted(_missing)}")
...
# N_TARGETS_PER_GROUP = 3   # capped to len(group) if the group is smaller
# SEED = 42
...
# OUT_DIR = ROOT / "CEBRA_NegPosDetach_AllGroups"
# SUMMARY_CSV = OUT_DIR / "normal_negpos_all_sessions_r2.csv"
...
#         print(f"[OK] {name}: train={X_train.shape} test={X_test.shape} neurons={X_train.shape[1]}")

#     return sessions


# # =============================================================================
# # NEURON-WISE STATS MATCHING
# # =============================================================================

# def column_stats(X: np.ndarray):
#     mu = X.mean(axis=0, keepdims=True).astype(np.float32)
#     sd = np.maximum(X.std(axis=0, keepdims=True).astype(np.float32), EPS)
#     return mu, sd


# def match_neuron_by_neuron(source: np.ndarray, target_mu: np.ndarray, target_sd: np.ndarray) -> np.ndarray:
#     """Rescales each column of `source` to have the mean/std given by
#     target_mu/target_sd (one value per column). target_mu/target_sd are
#     passed explicitly (rather than derived from a same-shape target array)
#     so this still works when the foreign pool came up short of the target
#     session's own neuron count -- see run_one_target_session."""
#     source = np.asarray(source, dtype=np.float32)
#     if source.shape[1] != target_mu.shape[1]:
#         raise ValueError(f"column mismatch: source={source.shape}, target stats={target_mu.shape}")
#     src_mu, src_sd = column_stats(source)
#     return (((source - src_mu) / src_sd) * target_sd + target_mu).astype(np.float32)


# def print_match_check(name: str, X: np.ndarray, target_mu: np.ndarray, target_sd: np.ndarray):
#     x_mu, x_sd = column_stats(X)
#     mean_err = float(np.max(np.abs(x_mu - target_mu)))
#     std_err = float(np.max(np.abs(x_sd - target_sd)))
#     print(f"{name}: {X.shape} | max mean error={mean_err:.6f} | max std error={std_err:.6f}")
...
# def choose_random_foreign_neurons(other_sessions: Dict[int, dict], n_neurons: int,
...
#               f"(< target session's {n_neurons} neurons) -- using all of them.")
#         n_neurons = len(pool)
...
# # =============================================================================
# # DECODER / R2 (no state_dict saved -- only the summary CSV is written to disk)
# # =============================================================================
...
# def train_decoder_and_r2(display_name, train_emb, test_emb, Y_train, Y_test):
...
#     seed_all(SEED)
#     decoder = TwoLayerMLP(
#         input_dim=train_emb.shape[1], hidden_dim=DECODER_HIDDEN_DIM,
#         output_dim=Ytr.shape[1], dropout_rate=DECODER_DROPOUT,
#     ).to(TORCH_DEVICE)

#     dataset = TensorDataset(torch.from_numpy(train_emb.astype(np.float32)),
#                             torch.from_numpy(Ytr.astype(np.float32)))
#     generator = torch.Generator().manual_seed(SEED)
#     loader = DataLoader(dataset, batch_size=DECODER_BATCH_SIZE, shuffle=True,
...
#     print(f"\nDecoder: {display_name} | epochs={DECODER_EPOCHS} | "
#           f"input={train_emb.shape[1]} | output={Ytr.shape[1]}")
#     for epoch in range(1, DECODER_EPOCHS + 1):
#         decoder.train()
#         for xb, yb in loader:
#             xb, yb = xb.to(TORCH_DEVICE), yb.to(TORCH_DEVICE)
#             optimizer.zero_grad(set_to_none=True)
#             loss = criterion(decoder(xb), yb)
#             loss.backward()
#             optimizer.step()
...
#     print(f"{display_name} TEST R2 per target:", r2_each)
#     print(f"{display_name} TEST Mean R2 = {mean_r2:.6f}")
...
# # =============================================================================
# # ONE TARGET SESSION -- NORMAL + NEGPOS only (no ACORN, no Jacobian, no plots)
# # =============================================================================
...
#           f"Y_train={Y_train.shape} Y_test={Y_test.shape} | neurons={n_neurons}")
...
#         f"{session_name} NORMAL", embed(normal_model, X_train), embed(normal_model, X_test),
#         Y_train, Y_test)
#     rows.append(dict(group=group, session=session_name, model="NORMAL",
#                      status="ok", mean_r2=normal_mean, r2_per_target=normal_r2.tolist()))
#     del normal_model
#     cleanup()

#     # ---------------- NEGPOS (detach) ----------------
#     mapping = choose_random_foreign_neurons(other_sessions, n_neurons, seed=SEED + target_day)

#     if len(mapping) == 0:
#         print(f"[WARN] {session_name}: no foreign neurons available at all -- skipping NEGPOS.")
#         rows.append(dict(group=group, session=session_name, model="NEGPOS",
#                          status="skipped_no_foreign_pool", mean_r2=float("nan"), r2_per_target=[]))
#         return rows

#     foreign_train_raw = build_foreign_matrix(other_sessions, mapping, split="train")
#     target_mu, target_sd = column_stats(X_train[:, :foreign_train_raw.shape[1]])
#     foreign_train_norm = match_neuron_by_neuron(foreign_train_raw, target_mu, target_sd)
#     print_match_check(f"{session_name} foreign_train_norm vs target", foreign_train_norm, target_mu, target_sd)
...
#         f"{session_name} NEGPOS", embed(negpos_model, X_train), embed(negpos_model, X_test),
#         Y_train, Y_test)
#     rows.append(dict(group=group, session=session_name, model="NEGPOS",
#                      status="ok", mean_r2=negpos_mean, r2_per_target=negpos_r2.tolist()))
#     del negpos_model
#     cleanup()
...
# # =============================================================================
# # SUMMARY CSV (rewritten after every session -- the ONLY file this script writes)
# # =============================================================================
...
#             print(f"[SKIP GROUP] {group}: fewer than 2 usable sessions, can't build a foreign pool.")
#             continue
