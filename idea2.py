...
# SESSION_SELECTION_SEED = 2026  # Change this to draw a different set of sessions.
# OUT_ROOT = ROOT / "ACORN_IDEA2_PERICH_COMPARE"
...
# def run_arm(cebra, label, config, seed, arrays, out, group, session, role):
#     x_train, x_valid, y_train, y_valid = arrays
...
#     try:
#         print(f"\nTRAIN {session} | {label} | encoder seed={seed}", flush=True)
#         seed_all(seed)
...
#         row = dict(group=group, session=session, role=role,
#                    n_neurons=x_train.shape[1], n_attacked_neurons=(
#                        config["adv_neuron_count"] if config["adv_neuron_count"] is not None
#                        else x_train.shape[1]),
#                    arm=label, seed=seed, decoder_seed=decoder_seed,
...
# def summarize(rows, arms, out, session):
...
# def select_sessions(data_dir, groups, count, selection_seed):
#     """Sample without replacement from existing files inside each declared range.

#     Local RNG does not read or change the training RNG. Missing candidates are
#     reported; fewer than count available files in any group stops the run.
#     """
#     if isinstance(count, bool) or not isinstance(count, int) or count < 1:
#         raise ValueError("SESSIONS_PER_GROUP must be a positive integer.")
#     rng = random.Random(selection_seed)
#     selection = {}
#     for group, days in groups.items():
#         candidates = [f"{group}{day}" for day in days]
#         if len(set(candidates)) != len(candidates):
#             raise ValueError(f"Duplicate session IDs in {group}.")
#         available = [s for s in candidates if (data_dir / f"{s}.npz").is_file()]
#         missing = [s for s in candidates if s not in available]
#         if len(available) < count:
#             raise FileNotFoundError(
#                 f"{group}: need {count} NPZ files, found {len(available)} in {data_dir}. "
#                 f"Missing: {missing}")
#         selected = rng.sample(available, count)
#         selection[group] = dict(selected=selected, available=available, missing=missing)
#         print(f"{group}: selected {selected} from {len(available)} files; "
#               f"missing {len(missing)}", flush=True)
#         if missing:
#             print(f"  Missing candidates (excluded): {missing}", flush=True)
#     return selection


# def summarize_all(rows, out):
#     """Pair by session AND training seed; average seeds before pooling sessions."""
#     fields = ["group", "session", "seed", "role", "arm", "n_neurons",
#               "n_attacked_neurons", "valid_mean_r2"]
#     with (out / "all_results.csv").open("w", newline="", encoding="utf-8") as handle:
#         writer = csv.DictWriter(handle, fieldnames=fields, extrasaction="ignore")
#         writer.writeheader()
#         writer.writerows(rows)
#     # Long format supports sessions with different label dimensions.
#     with (out / "all_r2_per_output.csv").open("w", newline="", encoding="utf-8") as handle:
#         writer = csv.writer(handle)
#         writer.writerow(["group", "session", "seed", "role", "output_index", "valid_r2"])
#         for row in rows:
#             for index, score in enumerate(row["valid_r2_per_output"]):
#                 writer.writerow([row["group"], row["session"], row["seed"],
#                                  row["role"], index, score])
#     pairs = {}
#     for row in rows:
#         key = (row["group"], row["session"], row["seed"])
#         pair = pairs.setdefault(key, {})
#         if row["role"] in pair:
#             raise ValueError(f"Duplicate result for {key}, {row['role']}")
#         pair[row["role"]] = row["valid_mean_r2"]
#     session_pairs = {}
#     paired_rows = []
#     for (group, session, seed), values in pairs.items():
#         if set(values) != {"baseline", "idea2"}:
#             raise ValueError(f"Incomplete pair for {session}, seed={seed}")
#         delta = values["idea2"] - values["baseline"]
#         paired_rows.append(dict(group=group, session=session, seed=seed,
#                                 baseline_r2=values["baseline"], idea2_r2=values["idea2"],
#                                 delta_idea_minus_baseline=delta))
#         session_pairs.setdefault((group, session), []).append(values)
#     with (out / "paired_deltas.csv").open("w", newline="", encoding="utf-8") as handle:
#         writer = csv.DictWriter(handle, fieldnames=["group", "session", "seed",
#                                "baseline_r2", "idea2_r2", "delta_idea_minus_baseline"])
#         writer.writeheader()
#         writer.writerows(paired_rows)
#     sessions = []
#     for (group, session), values in session_pairs.items():
#         baseline = float(np.mean([p["baseline"] for p in values]))
#         idea = float(np.mean([p["idea2"] for p in values]))
#         sessions.append(dict(group=group, session=session, n_seeds=len(values),
#                              baseline_r2=baseline, idea2_r2=idea,
#                              delta_idea_minus_baseline=idea - baseline))

#     def aggregate(items):
#         return dict(n_sessions=len(items),
#                     baseline_mean_r2=float(np.mean([s["baseline_r2"] for s in items])),
#                     idea2_mean_r2=float(np.mean([s["idea2_r2"] for s in items])),
#                     mean_delta=float(np.mean([s["delta_idea_minus_baseline"] for s in items])),
#                     idea2_wins=sum(s["delta_idea_minus_baseline"] > 0 for s in items))

#     groups = {group: aggregate([s for s in sessions if s["group"] == group])
#               for group in dict.fromkeys(s["group"] for s in sessions)}
#     save_json(out / "overall_summary.json", dict(
#         evaluation_split="validation", aggregation="mean over seeds, then equal-weight sessions",
#         sessions=sessions, groups=groups, overall=aggregate(sessions)))
#     with (out / "group_summary.csv").open("w", newline="", encoding="utf-8") as handle:
#         writer = csv.DictWriter(handle, fieldnames=["group", "n_sessions", "baseline_mean_r2",
#                                "idea2_mean_r2", "mean_delta", "idea2_wins"])
#         writer.writeheader()
#         writer.writerows(dict(group=group, **values) for group, values in groups.items())
#     fig, ax = plt.subplots(figsize=(max(10, len(sessions) * 0.7), 5))
#     positions = np.arange(len(sessions))
#     ax.bar(positions - 0.18, [s["baseline_r2"] for s in sessions], 0.36, label="Baseline")
#     ax.bar(positions + 0.18, [s["idea2_r2"] for s in sessions], 0.36, label="Idea 2")
#     ax.set_xticks(positions, [s["session"] for s in sessions], rotation=45, ha="right")
#     ax.set_ylabel("Validation mean R2 (averaged over training seeds)")
#     ax.axhline(0, color="black", linewidth=0.6)
#     ax.legend()
#     fig.tight_layout()
#     fig.savefig(out / "R2_all_sessions.png", dpi=250, bbox_inches="tight")
#     plt.close(fig)
#     print("\nGROUP RESULTS:", json.dumps(groups, indent=2), flush=True)
#     print("OVERALL:", aggregate(sessions), flush=True)


# def main():
#     if not SEEDS or len(set(SEEDS)) != len(SEEDS):
#         raise ValueError("SEEDS must contain at least one seed, with no duplicates.")
#     # Check all seven groups before starting any expensive training.
#     selection = select_sessions(PERICH_DATA_DIR, PERICH_DAYS,
#                                 SESSIONS_PER_GROUP, SESSION_SELECTION_SEED)
#     cebra = load_cebra_fork()
#     stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S_%f")
#     out = OUT_ROOT / stamp
#     out.mkdir(parents=True, exist_ok=False)
#     save_json(out / "selected_sessions.json", dict(
#         selection_seed=SESSION_SELECTION_SEED, sessions_per_group=SESSIONS_PER_GROUP,
#         sampling="without replacement, existing files within PERICH_DAYS",
#         declared_days={group: list(days) for group, days in PERICH_DAYS.items()}, groups=selection))
#     save_json(out / "run_config.json", dict(
#         fork=str(CEBRA_DIR), imported_cebra=str(cebra.__file__),
#         data_directory=str(PERICH_DATA_DIR), seeds=list(SEEDS),
#         common_encoder_config=common_encoder_config(),
#         subset=dict(count=IDEA2_NEURON_COUNT, fraction=IDEA2_NEURON_FRACTION,
#                     selection=IDEA2_SELECTION),
#         warmup=False, training="independent encoder and decoder per session/seed/arm",
...
#     ))
#     all_rows = []
#     for group, entry in selection.items():
#         for session in entry["selected"]:
#             print(f"\n===== SESSION {session} =====", flush=True)
#             npz_path = PERICH_DATA_DIR / f"{session}.npz"
#             arrays = load_data(npz_path)
#             try:
#                 arms = experiment_configs(arrays[0].shape[1])
#                 session_out = out / group / session
#                 session_out.mkdir(parents=True, exist_ok=False)
#                 save_json(session_out / "session_config.json", dict(
#                     group=group, session=session, data_path=str(npz_path),
#                     data_shapes=[list(a.shape) for a in arrays], arms=dict(arms)))
#                 rows = []
#                 for seed in SEEDS:
#                     for role, (label, config) in zip(("baseline", "idea2"), arms):
#                         row = run_arm(cebra, label, config, seed, arrays, session_out,
#                                       group, session, role)
#                         rows.append(row)
#                         all_rows.append(row)
#                         save_json(session_out / "results_so_far.json", rows)
#                         save_json(out / "results_so_far.json", all_rows)
#                 summarize(rows, arms, session_out, session)
#                 summarize_all(all_rows, out)
#             finally:
#                 del arrays
#                 cleanup()
#     print("\nAll selected sessions completed. Saved:", out, flush=True)
...
SESSION = "C-CO0"
NPZ_PATH = PERICH_DATA_DIR / f"{SESSION}.npz"
OUT_ROOT = ROOT / "ACORN_IDEA2_CCO12_COMPARE_TESTTTTT"
