...
def smooth_with_grid(spikes_np, sigma_ms, bin_width_ms=25.0, sqrt=False):
    """
    Wrapper so sigma=0.0 returns raw data safely.
    """
    spikes_np = np.asarray(spikes_np, dtype=np.float32)
    if sigma_ms <= 0:
        if sqrt:
            return np.sqrt(spikes_np, dtype=np.float32)
...
def reduce_attr_to_matrix(attr_tensor, total_neurons):
    """
    Converts attribution tensor to a 2D matrix [latent_dim, neurons] whenever possible.
    """
    ...
    return mat.cpu().numpy().astype(np.float32)
...
for name in names:
    ...
    for smooth_sigma_ms in SMOOTH_SIGMA_GRID_MS:
        sigma_tag = f"{smooth_sigma_ms:g}".replace(".", "p")
        ...
        for adv in [False, True]:
            ...
            # Save a single-model small preview figure if needed
            all_rows.append({
                "dataset": name,
                "sigma_ms": smooth_sigma_ms,
                "model": model_name,
        ...
        # Save the 2x2 panel: clean/acorn × Jf/Jf-inv
...
summary_csv = os.path.join(out_dir, "rat_smooth_full_results_summary.csv")
