"""Compare original puzzle, shared-tile puzzle, and frozen random features.

Place next to the NEW puzzle.py from the same archive. Examples:
    python -u run_cco16.py --variant random
    python -u run_cco16.py --variant window --epochs 1000
    python -u run_cco16.py --variant tiles --epochs 1000

Same encoder receptive field, initialization seed, decoding architecture,
label alignment and NPZ train/validation split. No extra 80/20 split.
Differences between window/tiles include objective construction, head size,
training span and encoder compute; this is NOT a compute-matched gap ablation.
For a gap ablation use tiles --gap-min 0 --gap-max 0 versus gaps 1..4.

Validation puzzle accuracy uses model.evaluate_puzzle, never the old
mask-concatenation evaluator. Fixed-weight TRAIN puzzle accuracy is also saved.
No labels or validation data enter encoder fitting. No checkpoint is selected
from validation results. All decoder R2 and PCA outputs use valid_data, even
if the file also contains test_data. PCA fits only training embeddings.
...
OUT_ROOT = ROOT / f"PUZZLE_GAPPED_{SESSION}_RESULTS"
SEED = 42

# Receptive field of one decoding window, in BOTH puzzle modes.
WINDOW_SIZE = 4
DELTA = 1
LATENT_DIM = 64
ENCODER_HIDDEN = 256
...
TILE_GAP = (1, 4)  # Discarded bins between intact tiles, not delta.
PUZZLE_EVAL_WINDOWS = 1024
PUZZLE_EVAL_BATCH_SIZE = 128  # Base puzzles; all 24 labels evaluated.
...
def main(variant="tiles"):
    ...
    model = puzzle(
        ...
        puzzle_mode="window" if variant == "window" else "tiles", tile_gap=TILE_GAP,
    )
    save_json(out / "run_config.json", dict(
        ...
        evaluation=dict(source="valid_data", max_windows=PUZZLE_EVAL_WINDOWS,
                        all_24_classes=True, seed=PUZZLE_EVAL_SEED),
    ...
    # Fail before long training if either split is too short for the new puzzle.
    if min(len(x_train), len(x_valid)) < model.training_span:
        raise ValueError(f"Both splits need at least {model.training_span} bins for puzzle evaluation.")
    ...
    for split, data in (("train", x_train), ("validation", x_valid)):
        metrics, details = model.evaluate_puzzle(
            data, max_windows=PUZZLE_EVAL_WINDOWS, permutations_per_window=24,
        ...
        print(f"\nPUZZLE {split.upper()} (fixed weights): "
              ...
              f"CE={metrics['cross_entropy']:.5f} | chance=4.17% | "
              f"base puzzles={metrics['n_windows']}", flush=True)
    if variant == "random":
    ...
    save_json(out / "summary.json", dict(variant=variant, encoder_epochs=model_epochs,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--variant", choices=("random", "window", "tiles"), default="tiles")
    ...
    parser.add_argument("--gap-min", type=int, default=TILE_GAP[0])
    parser.add_argument("--gap-max", type=int, default=TILE_GAP[1])
    ...
    PUZZLE_EPOCHS, PUZZLE_LR = args.epochs, args.lr
    PUZZLE_BATCH_SIZE, WINDOW_SIZE = args.batch_size, args.window_size
    TILE_GAP, SEED = (args.gap_min, args.gap_max), args.seed
    PUZZLE_EVAL_SEED = SEED + 200000
