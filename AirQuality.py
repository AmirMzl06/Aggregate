...
def compute_jacobian(model, X_train, tag):
    ...
    plot_attr(jf, os.path.join(IMG, f"{tag}_JC.png"), f"{tag} Full Train JC")
    plot_attr(jfi, os.path.join(IMG, f"{tag}_JC_INV.png"), f"{tag} Full Train JC inverse")
...
def train_decoder_forecasting(model, X_in_train, X_in_test, Y_tg_train, Y_tg_test, tag):
...
def train_reduced(X_train_full, Y_train_full, X_test_full, Y_test_full, time_train, source, attr_name, features, retrain_adv):
    ...
    r2 = train_decoder_forecasting(
        ...
        Y_tg_train=Y_train_full, Y_tg_test=Y_test_full, 
        tag=tag
    )
    
    print(f"REDUCED FORECASTING | Src: {source} | Attr: {attr_name} | Retrain: {'ACORN' if retrain_adv else 'CEBRA'} | R2: {r2:.4f}")
    clean(model)
    return r2
...
def main():
    X_raw = load_air()
    time_raw = time_label(len(X_raw))
    
    X_in = X_raw[:-FUTURE_SHIFT]
    time_in = time_raw[:-FUTURE_SHIFT]
    
    Y_target = X_raw[FUTURE_SHIFT:]
    
    split_idx = int(len(X_in) * 0.8)
    
    X_train_full = X_in[:split_idx]
    X_test_full  = X_in[split_idx:]
    
    Y_train_full = Y_target[:split_idx]
    Y_test_full  = Y_target[split_idx:]
    
    time_train   = time_in[:split_idx]
    
    results = {}
    
    print("\n================ FULL CEBRA (Forecasting t+1) ================")
    results["CEBRA"] = train_full(
        X_train=X_train_full, Y_train=Y_train_full, 
        time_train=time_train, 
        X_test=X_test_full, Y_test=Y_test_full, 
        tag="full_CEBRA", adv=False
    )
    
    print("\n================ FULL ACORN (Forecasting t+1) ================")
    results["ACORN"] = train_full(
        X_train=X_train_full, Y_train=Y_train_full, 
        time_train=time_train, 
        X_test=X_test_full, Y_test=Y_test_full, 
        tag="full_ACORN", adv=True
    )
    
    rows = []
    print("\n================ 8 STATES ABLATION FORECASTING ================")
        ...
        for attr in ["JC", "JC_INV"]:
            ...
            for retrain_adv in [False, True]:
                ...
                rows.append({
                    ...
                    "k": len(topk),
                    "r2_forecasting_t1": r2
                })
                
    df = pd.DataFrame(rows)
    path = os.path.join(OUT, "AirQuality_topK_8states_forecasting_summary.csv")
