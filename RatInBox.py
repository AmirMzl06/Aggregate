...
def _forward_split(model, x):
    """Forward pass preserving multi-objective outputs."""
    prev = getattr(model, "split_outputs", None)
    if prev is not None:
        model.split_outputs = True
    z = model(x)
    if prev is not None:
        model.split_outputs = prev
    return z
...
def build_adversarial(model, batch, eps, alpha, steps, lo, hi,
    ...
    with torch.no_grad():
        ...
        for bi, b in enumerate(batches):
            ...
            # xCEBRA multiobjective batches contain lists of samples/objectives.
            # Convert them back into a proper batch before embedding.
            if isinstance(b.positive, (list, tuple)):
                print("positive raw shapes:",
                      [tuple(x.shape) for x in b.positive])
                print("negative raw shapes:",
                      [tuple(x.shape) for x in b.negative])

                pos_x = torch.stack([x for x in b.positive], dim=0)
                neg_x = torch.stack([x for x in b.negative], dim=0)

                z_pos.append(_forward_full(model, pos_x.detach()))
                z_neg.append(_forward_full(model, neg_x.detach()))

            else:
                print("positive raw shape:", tuple(b.positive.shape))
                print("negative raw shape:", tuple(b.negative.shape))
        ...
        for i, b in enumerate(batches):
            ...
            if objective == "vat":
                ...
                for obj_idx in range(len(ranges)):
                    ...
                    if DEBUG_ATTACK_SHAPES:
                        print("FINAL LOSS SHAPES:",
                              tuple(z_obj.shape),
                              tuple(pos_obj.shape),
                              tuple(neg_obj.shape))
