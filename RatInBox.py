...
CLAMP_TO_DATA = True                    # spikes are non-negative counts
...
def rebuild_batch(batch, new_refs, was_single):
    out = []
    for b, r in zip(*as_batch_list(batch)[0:1] and (as_batch_list(batch)[0], new_refs)):
        nb = copy.copy(b)
        try:
            nb.reference = r
        except Exception:
...
def train_arm(arm_name, seed, neural, position):
    ...
    attack_kwargs = dict(eps=eps, alpha=alpha, steps=ADV_STEPS,
                         lo=float(neural.min()), hi=float(neural.max()),
    ...
    print(f"\n=== arm={arm_name} seed={seed} lam={cfg['lam']} adv={cfg['adv']} "
          f"double={cfg['double']} eps={eps:.4f} alpha={alpha:.4f} dev={DEVICE}")
    t0 = time.time()
...
# ALPHA_RULE    = 0.2                     # alpha = ALPHA_RULE * eps  (saturates the ball)
# ATTACK_OBJ    = "infonce"               # "infonce" (ascend the training loss) | "vat"
# CLAMP_TO_DATA = False                    # spikes are non-negative counts
