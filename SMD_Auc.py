...
# ------------------------------------------------------------
# FIX #1: cleanup_cuda was called but never defined. This made
# the whole `evaluate_one_machine` call raise a NameError right
# after training the *clean* CEBRA model, which meant:
#   - the ACORN (adv=True) model was never even trained
#   - the results already computed for that machine were lost,
#     because the function crashed before it could `return all_rows`
# ------------------------------------------------------------
def cleanup_cuda(cebra_model=None):
    """Free GPU/CPU memory held by a trained CEBRA model."""
    try:
        if cebra_model is not None and hasattr(cebra_model, "solver_"):
            del cebra_model.solver_
    except Exception:
        pass
    gc.collect()
    if torch.cuda.is_available():
...
# ------------------------------------------------------------
# FIX #2: batch_size was computed as min(attr_batch_size, len(x_np)),
# but the jacobian-based-batched attribution method internally
# consumes part of the sequence for the model's receptive field
# (for offset10-model this is 9 timesteps). So the *usable* length
# is len(x_np) - offset_consumed, which can be smaller than
# batch_size even after the original min() clamp. This is exactly
# why short/variable-length interpretation_label intervals failed.
# ------------------------------------------------------------
def get_model_offset_length(trained_model) -> int:
    """
    Total number of timesteps consumed by the model's receptive
    field (left context + right context). For offset10-model this
    is typically 9 (10 - 1).
    """
    try:
        offset = trained_model.get_offset()
...
def compute_attr_scores(cebra_model, x_np: np.ndarray, n_features: int, attr_batch_size: int) -> Dict[str, np.ndarray]:
    ...
    offset_len = get_model_offset_length(trained_model)
    usable_len = len(x_np) - offset_len

    if usable_len < 1:
        raise ValueError(
            f"Interval too short for model receptive field: "
            f"len(x_np)={len(x_np)}, offset_consumed={offset_len}, usable={usable_len}"
        )
    ...
    batch_size = max(1, min(attr_batch_size, usable_len))
