"""Temporal jigsaw learning: shared encoders on intact, separated time tiles.

Requires NumPy and PyTorch >=2.0. Inputs are (time, neurons) float arrays.
    from puzzle import puzzle
    model = puzzle(window_size=4, puzzle_mode="tiles", tile_gap=(1, 4),
                   max_epochs=1000, learning_rate=3e-4)
    model.fit(X_train)
    z, times = model.transform(X_valid, pad=False, return_indices=True)
    metrics = model.evaluate_puzzle(X_valid)

NEW default puzzle_mode="tiles": four nonoverlapping chronological tiles, each
window_size bins long. Internal order is preserved. A shared CEBRA-like encoder
processes each tile independently; ONLY the 4 latent vectors are permuted and
concatenated for a 24-class head. There are no cross-tile convolutions, time
indices, gap lengths, or selected-slot masks in that head. tile_gap=(min,max)
counts discarded bins BETWEEN tiles, sampled independently at every visit.
With W=4 and gaps=(1,4), actual puzzle spans are 19..28 bins. Each epoch visits
all starts eligible for the MAXIMUM span 28. Unused trailing bins are discarded.
Never infer tile_gap from delta: delta is for the original window mode only.

puzzle_mode="window" retains the supplied original implementation: shuffle four
individual slots [s,s+d,s+2d,s+3d] INSIDE one W-bin input; classify from one latent
plus a selected-slot mask. _draw_plan/_make_puzzles retain their old meaning.
Do not use that old evaluator on tiles mode; use model.evaluate_puzzle instead.

In BOTH modes transform encodes one natural W-bin window to output_dimension
features; it does not use the permutation head or the larger training span.
The encoder architecture is unchanged: valid convolutions, no temporal average,
no smoothing, no input normalization, no training padding. normalize is only
latent L2 normalization. No noise or neural channel shuffling is added.
With pad=False, window starting at j aligns to label j+W//2; pad=True uses edge
padding per sequence and gives one output per input bin. These are noncausal
centered contexts (W=4 uses t-2..t+1).

Split recordings BEFORE fit. A single 2D array is one continuous recording;
pass a list of trial arrays to avoid crossing trials. Each training/evaluation
sequence must contain at least training_span bins, but transform needs only W.
One epoch visits all eligible starts once in random order, with fresh puzzles.
permutations_per_window=1 samples a NEW label per visit, not one fixed label.
For tiles mode, extra permutations reuse features and only expand head compute.

fit ALWAYS resets weights; max_epochs=0 gives a frozen random-encoder control.
History reports online training accuracy. evaluate_puzzle uses fixed weights,
all 24 classes per sampled quartet by default, and restores module train modes.
Its batch_size counts base puzzles, not permutation copies. Sampled overlapping
windows are correlated; puzzle accuracy is not proof of behavioral usefulness.
Version-1 window-mode checkpoints can be loaded. Save does not include optimizer
state, so this module does not support resuming a training run.

Inspired by Noroozi & Favaro (2016), https://arxiv.org/abs/1603.09246.
This is a hypothesis about reducing local shortcuts, not a guarantee about R2.
Quick real-PyTorch test: python puzzle.py --self-test
"""
...
def _draw_tile_plan(batch, window_size, tile_gap, count, rng):
    ...
    Return one (B,4) set of relative tile starts and B*count labels. Gaps count
    excluded bins, not center spacing. Each epoch/visit draws a new plan.
    """
...
def _arrange_features(features, labels, count):
    ...
    Shuffling after encoding is equivalent to encoding shuffled tiles because
    the encoder has shared weights and no cross-tile operations. For count>1
    we reuse each tile's features/dropout realization, saving encoder compute.
    """
...
class puzzle:
    """Fit/transform estimator with 'tiles' (new) and 'window' (original) modes.

    window_size is ALWAYS the receptive field used for decoding. In tiles mode
    four intact windows train the shared encoder jointly; the head gets 4*D
    features. In window mode, delta selects single bins and the head gets D+W.
    max_epochs=0 initializes the encoder without learning. fit resets weights.
    No behavioral labels enter fit or evaluate_puzzle.
    """
    def __init__(self, window_size=10, delta=1, output_dimension=64,
                 ...
                 puzzle_mode="tiles", tile_gap=(1, 4)):
        ...
        if puzzle_mode not in ("window", "tiles"):
            raise ValueError("puzzle_mode must be 'window' (original) or 'tiles' (shared encoder).")
        ...
        if self.puzzle_mode == "tiles" and self.delta != (1,):
            raise ValueError("delta is only used in window mode; in tiles mode use tile_gap and leave delta=1.")

    @property
    def training_span(self):
        """Maximum raw span needed to sample a puzzle; encoder RF stays W."""
        return (self.window_size if self.puzzle_mode == "window"
                else 4 * self.window_size + 3 * self.tile_gap[1])
    ...
    def get_params(self):
        names = ("window_size", "delta", "output_dimension", "num_hidden_units",
                 ...
                 "device", "random_state", "verbose", "puzzle_mode", "tile_gap")
    ...
    def _build(self, channels):
        ...
        head_input = (self.output_dimension + self.window_size if self.puzzle_mode == "window"
                      else 4 * self.output_dimension)
        self.head_ = nn.Sequential(
            nn.Linear(head_input, self.head_hidden_units),
            nn.GELU(), nn.Linear(self.head_hidden_units, 24),
        ).to(self.device_)

    def _tile_logits(self, windows, starts, labels, count):
    ...
    def fit(self, X):
        ...
        self.n_steps_ = 0
        if self.verbose:
            print(f"Puzzle: mode={self.puzzle_mode}, {self.n_windows_} valid starts, "
                  f"encoder_window={self.window_size}, training_span={self.training_span}, "
                  f"delta={self.delta}, tile_gap={self.tile_gap}, "
                  f"{self.permutations_per_window} permutation(s)/puzzle, device={self.device_}", flush=True)
            if self.max_epochs == 0:
                print("Frozen random encoder: initialization only, zero optimizer steps.", flush=True)
    ...
    def evaluate_puzzle(self, X, *, max_windows=1024, permutations_per_window=24,
                        batch_size=128, random_state=200042, return_details=False):
        """Fixed-weight CE/accuracy with the checkpoint's SAME puzzle construction.

        Select up to max_windows distinct raw-span starts across all sequences.
        One quartet per start, all 24 permutations by default. Overlapping starts
        are correlated observations, not independent statistical replicates.
        batch_size counts base puzzles: tiles mode encodes 4*batch_size windows;
        window mode encodes permutations_per_window*batch_size windows.
        No training, labels, input normalization, or padded evaluation windows.
        """
        self._check_fitted()
        ...
        if self.puzzle_mode == "tiles":
            positions, labels = _draw_tile_plan(n, self.window_size, self.tile_gap, count, rng)
        else:
            slots, labels = _draw_plan(n, self.window_size, self.delta, count, rng)
            positions = slots.reshape(n, count, 4)[:, 0, :]
        predictions = np.empty(n * count, dtype=np.int64)
        losses = np.empty(n * count, dtype=np.float32)
        encoder_mode, head_mode = self.encoder_.training, self.head_.training
        self.encoder_.eval()
        self.head_.eval()
        try:
            with torch.inference_mode():
                for begin in range(0, n, size):
                    ...
                    if self.puzzle_mode == "tiles":
                        logits, target = self._tile_logits(windows, positions[begin:end], label_batch, count)
                    else:
                        xp, mask, target = _make_puzzles(windows, slots[begin * count:end * count],
                                                         label_batch, count)
                        logits = self.head_(torch.cat((self.encoder_(xp), mask), dim=1))
            ...
            self.encoder_.train(encoder_mode)
            self.head_.train(head_mode)
        confusion = np.zeros((24, 24), dtype=np.int64)
        np.add.at(confusion, (labels, predictions), 1)
        counts = confusion.sum(axis=1)
        correct = labels == predictions
        metrics = dict(
            puzzle_mode=self.puzzle_mode, window_size=self.window_size,
            training_span=self.training_span, tile_gap=list(self.tile_gap), delta=list(self.delta),
            ...
            class_counts=counts.tolist(), confusion_matrix=confusion.tolist(),
            per_class_accuracy=[float(confusion[i, i] / v) if v else None for i, v in enumerate(counts)],
            identity_accuracy=float(correct[labels == 0].mean()) if np.any(labels == 0) else None,
            nonidentity_accuracy=float(correct[labels != 0].mean()) if np.any(labels != 0) else None,
            sampling="Distinct span starts; one quartet per start; no cross-trial samples; no padding.",
        )
        ...
        details = dict(
            sequence_ids=sequence_ids, window_starts=window_starts,
            relative_positions=positions, absolute_positions=window_starts[:, None] + positions,
            true_class=labels.reshape(n, count), predicted_class=predictions.reshape(n, count),
            cross_entropy=losses.reshape(n, count), permutation_table=PERMUTATIONS.copy(),
        )
    ...
    def save(self, path):
        ...
        self._check_fitted()
        payload = dict(
            format_version=2, params=self.get_params(), n_features=self.n_features_in_,
...
def _self_test():
    ...
    try:
        ...
        model = puzzle(window_size=10, delta=(1, 2, 3), puzzle_mode="window", output_dimension=6,
        ...
        # All tile permutations preserve internal time order and channel identity.
        plan_rng = np.random.default_rng(91)
        starts, labels = _draw_tile_plan(2, 4, (1, 4), 24, plan_rng)
        raw = torch.arange(2 * 3 * 28, dtype=torch.float32).reshape(2, 3, 28)
        tiles = _extract_tiles(raw, starts, 4)
        for b in range(2):
            for tile in range(4):
                torch.testing.assert_close(tiles[b, tile], raw[b, :, starts[b, tile]:starts[b, tile] + 4])
        arranged, target = _arrange_features(tiles.reshape(2, 4, -1), labels, 24)
        for b in range(2):
            for label, permutation in enumerate(PERMUTATIONS):
                expected = torch.stack([tiles[b, int(i)] for i in permutation]).flatten()
                torch.testing.assert_close(arranged[b * 24 + label], expected)
        assert target.tolist() == list(range(24)) * 2

        # New shared architecture: real training, gradients, eval sampling and IO.
        trials = [rng.normal(size=(43, 3)).astype(np.float32),
                  rng.normal(size=(35, 3)).astype(np.float32)]
        copies = [x.copy() for x in trials]
        params = dict(window_size=4, output_dimension=6, num_hidden_units=8,
                      head_hidden_units=16, batch_size=8, device="cpu", verbose=False,
                      tile_gap=(1, 4), random_state=71)
        frozen = puzzle(**params, max_epochs=0).fit(trials)
        old_frozen = puzzle(**params, max_epochs=0, puzzle_mode="window").fit(trials)
        for a, b in zip(frozen.encoder_.parameters(), old_frozen.encoder_.parameters()):
            torch.testing.assert_close(a, b, rtol=0, atol=0)
        model = puzzle(**params, max_epochs=2).fit(trials)
        assert model.training_span == 28 and model.n_windows_ == 24 and model.n_steps_ == 6
        assert all(row['n_puzzles'] == 24 for row in model.history_)
        assert frozen.n_steps_ == 0 and frozen.history_ == []
        assert any(not torch.equal(a, b) for a, b in zip(model.encoder_.parameters(), frozen.encoder_.parameters()))
        assert any(p.grad is not None and p.grad.abs().sum().item() > 0 for p in model.encoder_.parameters())
        assert model.head_[0].in_features == 4 * params['output_dimension']
        assert all(np.array_equal(a, b) for a, b in zip(trials, copies))

        # Gap values never enter encoder/head; tile features cannot inspect a neighbor.
        fixed_starts = np.asarray([[0, 5, 11, 18]])
        sample = torch.randn(1, 3, 28)
        changed_gap = sample.clone()
        changed_gap[:, :, 4] = 1e6
        with torch.inference_mode():
            z = model.encoder_(_extract_tiles(sample, fixed_starts, 4).reshape(4, 3, 4))
            z_gap = model.encoder_(_extract_tiles(changed_gap, fixed_starts, 4).reshape(4, 3, 4))
            torch.testing.assert_close(z, z_gap, rtol=0, atol=0)
            changed_tile = sample.clone()
            changed_tile[:, :, :4] += 9
            z_changed = model.encoder_(_extract_tiles(changed_tile, fixed_starts, 4).reshape(4, 3, 4))
            torch.testing.assert_close(z[1:], z_changed[1:], rtol=0, atol=0)

        for evaluation_model in (model, old_frozen):
            evaluation_model.encoder_.train()
            evaluation_model.head_.eval()
            torch_rng = torch.random.get_rng_state().clone()
            m1, d1 = evaluation_model.evaluate_puzzle(trials, max_windows=12, batch_size=1, return_details=True)
            m2, d2 = evaluation_model.evaluate_puzzle(trials, max_windows=12, batch_size=7, return_details=True)
            assert evaluation_model.encoder_.training and not evaluation_model.head_.training
            assert torch.equal(torch_rng, torch.random.get_rng_state())
            assert m1['class_counts'] == [12] * 24 and m1['n_puzzles'] == 288
            np.testing.assert_allclose(m1['cross_entropy'], m2['cross_entropy'], atol=1e-6)
            for key in ('window_starts', 'relative_positions', 'true_class', 'predicted_class'):
                np.testing.assert_array_equal(d1[key], d2[key])
            for seq, start in zip(d1['sequence_ids'], d1['window_starts']):
                assert start >= 0 and start + evaluation_model.training_span <= len(trials[int(seq)])
            with tempfile.TemporaryDirectory() as tmp:
                path = Path(tmp) / 'checkpoint.pt'
                evaluation_model.save(path)
                loaded = puzzle.load(path, device="cpu")
                a, ia = evaluation_model.transform(trials[0], pad=False, return_indices=True)
                b, ib = loaded.transform(trials[0], pad=False, return_indices=True)
                assert a.shape == (40, 6)
                np.testing.assert_array_equal(ia, np.arange(2, 42))
                np.testing.assert_array_equal(ia, ib)
                np.testing.assert_allclose(a, b, rtol=1e-5, atol=1e-6)
                if evaluation_model.puzzle_mode == "window":
                    payload = torch.load(path, map_location="cpu", weights_only=True)
                    payload['format_version'] = 1
                    payload['params'].pop('puzzle_mode')
                    payload['params'].pop('tile_gap')
                    torch.save(payload, path)
                    legacy = puzzle.load(path, device="cpu")
                    assert legacy.puzzle_mode == "window"
                    np.testing.assert_allclose(legacy.transform(trials[0], pad=False), a, rtol=1e-5, atol=1e-6)
        print("PASS: both puzzle modes, all 24 permutations, CPU fit/autograd, "
              "gap exclusion, tile isolation, frozen control, evaluation batching, "
              "trial boundaries, label alignment and v1/v2 save/load.")
    finally:
