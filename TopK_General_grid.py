# ### General one session , grid offset
# ## General one session, OFFSET grid with fixed epsilon
...
# OUT = f"TopK_Percentage_{SESSION}_OffsetGrid_Eps0.3"
...
# def build_acorn(offset, eps=ADV_EPSILON):
#     print("\nBuilding ACORN")
#     print("offset =", offset)
#     print("eps    =", eps)
#     return CEBRA(
...
# def train_acorn_label(X_train, Y_train, offset):
#     print("\n" + "=" * 100)
#     print(f"TRAINING ACORN + LABEL | offset={offset} | eps={ADV_EPSILON:.6f}")
#     print("=" * 100)
#     model = build_acorn(offset, ADV_EPSILON)
#     model.fit(X_train, Y_train)
#     return model

# def train_clean_label(X_train, Y_train, offset):
#     print("\n" + "=" * 100)
#     print(f"TRAINING CLEAN + LABEL | offset={offset}")
#     print("=" * 100)
#     model = build_clean(offset)
#     model.fit(X_train, Y_train)
#     return model
...
#         if "jf" not in result:
#             raise RuntimeError("No 'jf' key in attribution result.\n" f"Available keys: {list(result.keys())}")
#         jf_chunk = orient_jacobian(result["jf"], n_neurons)
#         weight = len(chunk)
#         jf_sum += jf_chunk * weight
#         total += weight
#         print(f"chunk {chunk_id + 1:02d}/" f"{len(starts):02d} done")
#         del method
#         del result
#         del jf_chunk
#         del inp
...
#             print(f"  epoch {epoch + 1}/" f"{MLP_EPOCHS}" f" | train MSE: {epoch_loss:.6f}")
#     del Zt
#     del Yt
#     del optimizer
#     cleanup()
...
#         r2s.append(r2)
#         print(f"{name} dim {i} R2: " f"{r2:.6f}")
#     mean_r2 = float(np.mean(r2s))
#     print(f"{name} Mean R2: " f"{mean_r2:.6f}")
#     return mean_r2

# def run_reduced_case(X_train_red, X_test_red, Y_train, Y_test, offset, tag):
#     model = build_clean(offset)
...
# def run_percentage_curve_for_ranking(score, X_train, X_test, Y_train, Y_test, offset, tag_prefix):
...
#         tag = f"{tag_prefix}_" f"{pct}pct"
#         r2 = run_reduced_case(X_train_red, X_test_red, Y_train, Y_test, offset, tag)
#         curve.append(r2)
#         print(f"  [{tag_prefix}] " f"pct={pct}% " f"k={k} " f"R2={r2:.4f}")
#     return curve, k_list

# def plot_comparison_curve(clean_curve, acorn_curve, offset):
#     plt.figure(figsize=(10, 7))
#     plt.plot(PERCENTAGES, clean_curve, marker="o", color="blue", label="Neurons ranked by CLEAN Jacobian")
#     plt.plot(PERCENTAGES, acorn_curve, marker="o", color="red", label="Neurons ranked by ACORN Jacobian " f"(offset={offset}, eps={ADV_EPSILON:.4f})")
#     plt.xlabel("Top-% neurons kept")
#     plt.ylabel("Decoder Mean R2")
#     plt.title("Plain CEBRA retrained on reduced neuron sets\n" f"CLEAN baseline vs ACORN " f"@ offset={offset}, eps={ADV_EPSILON:.4f}")
#     plt.legend()
#     plt.grid(alpha=0.3)
#     path = os.path.join(OUT, f"R2_vs_percentage_offset_{offset}.png")
#     plt.savefig(path, dpi=300, bbox_inches="tight")
#     plt.close()
#     print("Saved:", path)

# def plot_overview(clean_curves_by_offset, acorn_curves_by_offset):
#     plt.figure(figsize=(12, 8))
#     for offset, curve in clean_curves_by_offset.items():
#         plt.plot(PERCENTAGES, curve, marker="o", linewidth=2.0, label=f"CLEAN-ranked | offset={offset}")
#     for offset, curve in acorn_curves_by_offset.items():
#         plt.plot(PERCENTAGES, curve, marker="o", linestyle="--", linewidth=2.0, label=f"ACORN-ranked | offset={offset} " f"| eps={ADV_EPSILON:.2f}")
#     plt.xlabel("Top-% neurons kept")
#     plt.ylabel("Decoder Mean R2")
#     plt.title("R2 vs percentage of neurons kept\n" f"OFFSET GRID {OFFSET_GRID} | " f"fixed epsilon={ADV_EPSILON:.2f}")
#     plt.legend(fontsize=8)
#     plt.grid(alpha=0.3)
#     path = os.path.join(OUT, "R2_vs_percentage_ALL_OFFSETS.png")
#     plt.savefig(path, dpi=300, bbox_inches="tight")
#     plt.close()
#     print("Saved:", path)
...
# def main():
#     print("\n" + "#" * 110)
#     print("TOP-K PERCENTAGE SWEEP x OFFSET GRID: " "CLEAN vs ACORN JACOBIAN RANKING")
#     print(f"FIXED EPSILON = {ADV_EPSILON:.4f}")
...
#     print("\nFixed epsilon:", ADV_EPSILON)
#     print("Offset grid:", OFFSET_GRID)
#     print("\n" + "=" * 110)
#     print("OFFSET GRID")
#     print("=" * 110)
...
#     all_rows = []
#     clean_curves_by_offset = {}
#     acorn_curves_by_offset = {}
#     for offset in OFFSET_GRID:
#         print("\n" + "#" * 130)
#         print(f"OFFSET GRID POINT: offset={offset}")
#         print(f"FIXED EPSILON: {ADV_EPSILON:.6f}")
#         print("#" * 130)
#         clean_model = train_clean_label(X_train, Y_train, offset)
#         clean_jf = compute_jacobian(clean_model, X_train, tag=f"CLEAN_LABEL_full_offset_{offset}")
#         del clean_model
#         cleanup()
#         clean_score = clean_jf.mean(axis=0)
#         print("\n" + "=" * 110)
#         print("BASELINE CURVE: " f"neurons ranked by CLEAN Jacobian | offset={offset}")
#         print("=" * 110)
...
#             Y_test,
#             offset=offset,
#             tag_prefix=f"CLEANranked_offset_{offset}",
#         )
#         clean_curves_by_offset[offset] = clean_curve
#         del clean_jf
#         del clean_score
#         cleanup()
#         acorn_model = train_acorn_label(X_train, Y_train, offset)
#         acorn_jf = compute_jacobian(acorn_model, X_train, tag=f"ACORN_LABEL_full_offset_{offset}_eps_{ADV_EPSILON:.2f}")
#         del acorn_model
#         cleanup()
#         acorn_score = acorn_jf.mean(axis=0)
#         print("\n" + "=" * 110)
#         print("ACORN CURVE: " f"neurons ranked by ACORN Jacobian | " f"offset={offset} | eps={ADV_EPSILON:.4f}")
#         print("=" * 110)
...
#             Y_test,
#             offset=offset,
#             tag_prefix=f"ACORNranked_offset_{offset}" f"_eps_{ADV_EPSILON:.2f}",
#         )
#         acorn_curves_by_offset[offset] = acorn_curve
#         plot_comparison_curve(clean_curve, acorn_curve, offset)
...
#         del acorn_score
#         cleanup()
#     plot_overview(clean_curves_by_offset, acorn_curves_by_offset)
#     summary_df = pd.DataFrame(all_rows)
#     csv_path = os.path.join(OUT, "topk_percentage_offset_grid_summary.csv")
...
SESSION = f"{DATASET_NAME}{DAY}"
NPZ_PATH = os.path.join(PERICH_DATA_DIR, f"{SESSION}.npz")
OUT = f"TopK_Percentage_{SESSION}_Aggregate"
os.makedirs(OUT, exist_ok=True)
...
DEVICE = "cuda_if_available"
PERCENTAGES = [1, 5, 15, 30, 50, 75, 90, 100]
EPSILON_GRID_FIXED = [0.1,0.3,0.5,0.75,1]
MLP_HIDDEN_DIM = 64
MLP_DROPOUT = 0.4
MLP_LR = 1e-3
MLP_WEIGHT_DECAY = 1e-4
